...
- **Anything with a standard name is given its name.** Operations, objects, matrices, theorems, inequalities, and the notational patterns themselves. A reader who knows something only by its formula cannot look it up, cannot search for it, and cannot ask another person about it. Name it where it is defined, even when the name contributes nothing to the mathematics — especially then, because that is exactly when it is easiest to leave out.
