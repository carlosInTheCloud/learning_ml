# Course 2 — Lab 7 — Random Forest vs XGBoost

## Before you run

**Watch the clock.** The script uses `time.time()` trackers. When you run it, notice how fast your CPU moves through the Random Forest grid search (parallel trees) compared to the XGBoost grid search (sequential boosting chain).

**Read the business output.** At the end, the script prints raw confusion matrices and precision/recall scores. Act as the business stakeholder: decide which model actually **wins** based on the cost of false positives vs. false negatives.

---

## Lab script

Paste and run the following **Python** (requires `scikit-learn` and `xgboost`):
...
Run the script above. When it finishes, compare the two confusion matrices.

Based on the raw numbers of **false positives** and **false negatives** from the Random Forest vs. the XGBoost model, which model would you choose to deploy to your cycling app? Give your **architectural/business justification** — not just which accuracy number is higher.